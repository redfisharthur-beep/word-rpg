...
func start_idle(sprite: Sprite2D, intensity := 1.0) -> void:
...
func play_combo(player: Sprite2D, enemy: Sprite2D, camera: Camera2D) -> void:
	...
	var enemy_origin := enemy.position
	var target := enemy_origin + Vector2(-175, 45)
	...
	var slash_angles := [-0.62, 0.48, -0.12]
	var damages := ["128", "146", "CRIT 312"]
	for i in range(3):
		_spawn_slash(enemy.position, slash_angles[i], i == 2)
		_spawn_damage(enemy.position, damages[i], i == 2)
		await _hit_reaction(enemy, enemy_origin, 14.0 + i * 6.0)
		await _shake_camera(camera, 5.0 + i * 2.5, 0.075 + i * 0.025)
		await get_tree().create_timer(0.035).timeout
...
func _spawn_slash(at_position: Vector2, angle: float, critical := false) -> void:
	var line := Line2D.new()
...
func _spawn_damage(at_position: Vector2, text: String, critical := false) -> void:
	var label := Label.new()
	label.text = text
...
func _shake_camera(camera: Camera2D, strength: float, duration: float) -> void:
	var steps := max(3, int(duration / 0.018))
