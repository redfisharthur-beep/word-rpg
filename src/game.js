...
function freshEnemy(s){return {name:s.enemy,art:s.art,icon:s.icon,hp:s.hp,currentHp:s.hp,atk:s.atk,def:s.def,poison:0,burn:0,boss:!!s.boss};}
...
function renderHome(){const r=ROLES[meta.role];return `<main class="home-screen"><div class="home-overlay"></div><section class="home-panel"><div class="logo-mark">WORD QUEST <span>V3</span></div><div class="home-copy"><small>AUTO BATTLE × ENGLISH</small><h1>答題養成，三回合驗證。</h1><p>每次先答題拿能力，再自動交戰 3 回合。技能升到 Lv.3、Lv.5 會進化。</p></div><div class="role-preview">${visual(r.art,r.icon,'home-role-art')}<div><b>${r.name}</b><span>${r.desc}</span></div></div><div class="role-row">${Object.entries(ROLES).map(([id,x])=>`<button class="role-btn ${id===meta.role?'active':''}" data-role="${id}">${x.icon}<b>${x.name}</b></button>`).join('')}</div><button class="fight-btn" data-action="start">FIGHT <span>→</span></button><div class="meta-row"><span>最佳連對 <b>${meta.bestStreak}</b></span><span>通關 <b>${meta.wins}</b></span><span>挑戰 <b>${meta.runs}</b></span></div></section></main>`;}
...
function playBattleRound(){if(screen!=='play'||run.battleRound<1)return;clearFx();if(run.regen){const before=run.hp;run.hp=Math.min(run.maxHp,run.hp+run.regen);if(run.hp>before)combatFx.hero=`+${run.hp-before}`;}
 const critLv=skillLevel('crit'),comboLv=skillLevel('combo'),fireLv=skillLevel('fire'),poisonLv=skillLevel('poison');
 ...
 let damage=Math.max(1,raw-run.enemy.def)+run.fire,combo=0,secondCombo=0,finisher=0;
 if(run.combo>0&&Math.random()<Math.min(.78,run.combo)){combo=Math.max(1,Math.round(run.atk*(comboLv>=3?.85:.6))-run.enemy.def);if(comboLv>=5)secondCombo=Math.max(1,Math.round(run.atk*.45)-Math.floor(run.enemy.def*.5));}
 if(crit&&critLv>=5)finisher=Math.max(1,Math.round(run.atk*.45));
 combatFx.proc=[crit?'💥 暴擊':'',combo?comboLv>=5?'➶ 二段連擊':'➶ 連擊':'',finisher?'✦ 終結追擊':'',run.fire?'🔥 灼燒':''].filter(Boolean).join(' · ');
 anim='hero-attack';render();setTimeout(()=>{const total=damage+combo+secondCombo+finisher;run.enemy.currentHp-=total;combatFx.enemy=`-${total}`;
   if(run.fire&&fireLv>=3){run.enemy.burn+=Math.max(1,Math.ceil(run.fire*.5));if(fireLv>=5&&run.battleRound===3){const burst=Math.max(2,run.fire*2);run.enemy.currentHp-=burst;combatFx.enemy=`-${total+burst}`;combatFx.proc=[combatFx.proc,'🔥 烈焰爆發'].filter(Boolean).join(' · ');}}
   if(run.poison){const poisonMult=poisonLv>=5?2:poisonLv>=3?1.4:1;const add=Math.max(1,Math.round(run.poison*poisonMult));run.enemy.poison+=add;combatFx.proc=[combatFx.proc,`☠️ +${add}`].filter(Boolean).join(' · ')}
   if(run.lifesteal){const heal=Math.max(1,Math.round(total*run.lifesteal));run.hp=Math.min(run.maxHp,run.hp+heal);combatFx.hero=`+${heal}`}
   anim='enemy-hit';render();setTimeout(()=>afterHeroAttack(),PAUSE.impact);},PAUSE.attack);}

function afterHeroAttack(){let dot=0;const tags=[];if(run.enemy.burn>0){dot+=run.enemy.burn;tags.push(`🔥 ${run.enemy.burn}`)}if(run.enemy.poison>0){dot+=run.enemy.poison;tags.push(`☠️ ${run.enemy.poison}`)}if(dot>0){run.enemy.currentHp-=dot;combatFx.enemy=`-${dot}`;combatFx.proc=tags.join(' · ')}
 if(run.enemy.currentHp<=0){anim='enemy-down';render();setTimeout(()=>finishBattleSet(true),PAUSE.defeat);return}
 clearFx();anim='enemy-turn';render();setTimeout(()=>{const preShield=Math.max(1,run.enemy.atk-run.def),incoming=Math.max(1,preShield-run.shield),blocked=Math.max(0,preShield-incoming);run.hp=Math.max(0,run.hp-incoming);combatFx.hero=`-${incoming}`;
   const counterLv=skillLevel('counter'),shieldLv=skillLevel('shield');let counter=0,thorns=0,heal=0;
   if(run.counter>0){counter=counterLv>=5?Math.max(1,Math.round(run.atk*run.counter)):Math.max(1,Math.round(run.atk*run.counter)-run.enemy.def);run.enemy.currentHp-=counter;combatFx.enemy=`-${counter}`;combatFx.proc=counterLv>=5?'↩️ 穿甲反擊':'↩️ 反擊';if(counterLv>=3){heal=Math.max(1,Math.round(counter*.25));run.hp=Math.min(run.maxHp,run.hp+heal);combatFx.hero=`-${incoming} / +${heal}`;combatFx.proc+=' · 🩸 回復';}}
