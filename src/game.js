...
const STAGES=[
 {id:1,name:'苔蘚小徑',enemy:'苔球獸',art:ASSETS.enemy.moss,icon:'🟢',hp:230,atk:10,def:2,trait:'🌿 纏藤',traitText:'每組第 2 回合纏住你，該回合傷害降低 30%',mechanic:'vine',reward:'擊破後回復 12% 生命'},
 {id:2,name:'霧林追獵',enemy:'霧角兔',art:ASSETS.enemy.rabbit,icon:'🐇',hp:360,atk:14,def:4,trait:'💨 殘影',traitText:'每次受到主攻擊時有 28% 機率閃避',mechanic:'dodge',reward:'擊破後回復 14% 生命'},
 {id:3,name:'古樹守門',enemy:'木甲蟲',art:ASSETS.enemy.beetle,icon:'🪲',hp:540,atk:18,def:7,trait:'🪨 硬殼',traitText:'前 2 回合額外 +6 防禦，第 3 回合硬殼裂開',mechanic:'shell',reward:'擊破後回復 16% 生命'},
...
const EVOLUTIONS={
 crit:[{level:3,name:'破甲暴擊',text:'暴擊倍率提升為 2.4 倍'},{level:5,name:'終結追擊',text:'暴擊後追加 45% 攻擊'}],
 combo:[{level:3,name:'強襲連擊',text:'連擊傷害提升至 85%'},{level:5,name:'二段連擊',text:'連擊觸發時再追加第二擊'}],
 fire:[{level:3,name:'灼熱餘燼',text:'火焰會留下持續灼燒'},{level:5,name:'烈焰爆發',text:'每組第 3 回合引爆火焰'}],
 poison:[{level:3,name:'濃縮劇毒',text:'中毒累積量提升 40%'},{level:5,name:'致命毒素',text:'中毒累積量提升為 2 倍'}],
 counter:[{level:3,name:'血性反擊',text:'反擊傷害可回復生命'},{level:5,name:'穿甲反擊',text:'反擊無視敵人防禦'}],
 shield:[{level:3,name:'荊棘護體',text:'格擋傷害時反彈部分傷害'},{level:5,name:'鋼鐵荊棘',text:'反彈傷害大幅提升'}],
};
...
function renderHome(){const r=ROLES[meta.role];return `<main class="home-screen"><div class="home-overlay"></div><section class="home-panel"><div class="logo-mark">WORD QUEST <span>V3</span></div><div class="home-copy"><small>AUTO BATTLE × ENGLISH</small><h1>答題養成，三回合驗證。</h1><p>每次先答題拿能力，再自動交戰 3 回合。技能會進化，怪物也各有戰鬥特性。</p></div><div class="role-preview">${visual(r.art,r.icon,'home-role-art')}<div><b>${r.name}</b><span>${r.desc}</span></div></div><div class="role-row">${Object.entries(ROLES).map(([id,x])=>`<button class="role-btn ${id===meta.role?'active':''}" data-role="${id}">${x.icon}<b>${x.name}</b></button>`).join('')}</div><button class="fight-btn" data-action="start">FIGHT <span>→</span></button><div class="meta-row"><span>最佳連對 <b>${meta.bestStreak}</b></span><span>通關 <b>${meta.wins}</b></span><span>挑戰 <b>${meta.runs}</b></span></div></section></main>`;}

function renderStageIntro(){const s=stage();return `<main class="stage-screen"><section class="stage-card"><small>STAGE ${s.id} / ${STAGES.length}</small><h1>${s.name}</h1>${visual(s.art,s.icon,'stage-enemy-art')}<h2>${s.enemy}</h2><p>${s.boss?'最後的 Boss 戰。前面累積的所有能力都會在這裡派上用場。':'先看敵人能力與特性，再答題強化自己；每次選完能力後自動交戰 3 回合。'}</p><div class="stage-stats three"><span>❤️ HP <b>${s.hp}</b></span><span>⚔️ 攻擊 <b>${s.atk}</b></span><span>🛡️ 防禦 <b>${s.def}</b></span></div><div class="build-line"><b>${s.trait}</b>　${s.traitText}</div><button class="fight-btn" data-action="enter-stage">開始答題 <span>→</span></button></section></main>`;}
...
function renderOwnedSkills(){const owned=Object.entries(run.skills).sort((a,b)=>b[1]-a[1]);if(!owned.length)return '<span class="empty-skill">尚未取得能力</span>';return owned.slice(0,7).map(([id,l],i)=>{const u=UPGRADES.find(x=>x.id===id),e=evolutionState(id,l).active;return `<span class="${i===0?'main-build':''}">${u?.icon||'✦'} ${u?.name||id} <b>Lv.${fmtLevel(l)}</b>${e?` · ${e.name}`:''}</span>`}).join('');}
...
function renderUpgrade(){const rate=pendingCorrect?1:.5;return `<main class="choice-screen"><div class="choice-head"><small>${pendingCorrect?'FULL POWER':'HALF POWER'}</small><h1>${pendingCorrect?'答對：三選一完整強化':'答錯：仍可三選一，但只有 50% 效果'}</h1><p>${pendingCorrect?'選完後會自動戰鬥 3 回合。':'學習不中斷，下一題還有機會追回來。'}</p></div><div class="upgrade-grid">${upgradeChoices.map(u=>{const cur=skillLevel(u.id),next=cur+(pendingCorrect?1:.5),note=evolutionNote(u.id,next);return `<button data-upgrade="${u.id}"><span>${u.icon}</span><div><em>${cur?`Lv.${fmtLevel(cur)} → Lv.${fmtLevel(next)}`:`NEW → Lv.${fmtLevel(next)}`}</em><b>${u.name}</b><small>${u.desc(Math.max(1,next))}${rate<1?' · 本次 50% 效果':''}${note?`<br>✦ ${note}`:''}</small></div></button>`}).join('')}</div><div class="build-line">${summarizeBuild()}</div></main>`;}

function summarizeBuild(){const owned=Object.entries(run.skills).sort((a,b)=>b[1]-a[1]);if(!owned.length)return '尚未成形';return owned.map(([id,l])=>{const u=UPGRADES.find(x=>x.id===id),e=evolutionState(id,l).active;return `${u?.icon||'✦'} ${u?.name||id} Lv.${fmtLevel(l)}${e?`【${e.name}】`:''}`}).join('　');}
...
function enemyRoundProfile(){const mech=run.enemy.mechanic,round=run.battleRound;let heroDamageMult=1,enemyDefBonus=0,evadeChance=0,enemyAtkMult=1,enemyHeal=0,label='';if(mech==='vine'&&round===2){heroDamageMult=.7;label='🌿 纏藤';}if(mech==='dodge'){evadeChance=.28;}if(mech==='shell'&&round<=2){enemyDefBonus=6;label='🪨 硬殼';}if(mech==='shadow'&&round===2){heroDamageMult=.75;label='🌑 暗影護幕';}if(mech==='shadow'&&round===3){enemyAtkMult=1.45;enemyHeal=10;label='👁️ 吞噬';}return {heroDamageMult,enemyDefBonus,evadeChance,enemyAtkMult,enemyHeal,label};}
function playBattleRound(){if(screen!=='play'||run.battleRound<1)return;clearFx();if(run.regen){const before=run.hp;run.hp=Math.min(run.maxHp,run.hp+run.regen);if(run.hp>before)combatFx.hero=`+${run.hp-before}`;}
 const profile=enemyRoundProfile(),critLv=skillLevel('crit'),comboLv=skillLevel('combo'),fireLv=skillLevel('fire'),poisonLv=skillLevel('poison');
 let raw=Math.round(run.atk*(1+run.rage*Math.max(0,run.streak-1))),crit=false;if(Math.random()<run.crit){raw=Math.round(raw*(critLv>=3?2.4:2));crit=true}
 const effectiveDef=run.enemy.def+profile.enemyDefBonus;let damage=Math.max(1,Math.round((raw-effectiveDef)*profile.heroDamageMult))+run.fire,combo=0,secondCombo=0,finisher=0;
 if(run.combo>0&&Math.random()<Math.min(.78,run.combo)){combo=Math.max(1,Math.round((run.atk*(comboLv>=3?.85:.6)-effectiveDef)*profile.heroDamageMult));if(comboLv>=5)secondCombo=Math.max(1,Math.round((run.atk*.45-Math.floor(effectiveDef*.5))*profile.heroDamageMult));}
 if(crit&&critLv>=5)finisher=Math.max(1,Math.round(run.atk*.45*profile.heroDamageMult));
 const dodged=profile.evadeChance>0&&Math.random()<profile.evadeChance;if(dodged){damage=0;combo=0;secondCombo=0;finisher=0;}
 combatFx.proc=[profile.label,dodged?'💨 殘影閃避':'',crit&&!dodged?'💥 暴擊':'',combo?comboLv>=5?'➶ 二段連擊':'➶ 連擊':'',finisher?'✦ 終結追擊':'',run.fire&&!dodged?'🔥 灼燒':''].filter(Boolean).join(' · ');
 anim='hero-attack';render();setTimeout(()=>{const total=damage+combo+secondCombo+finisher;run.enemy.currentHp-=total;combatFx.enemy=dodged?'MISS':`-${total}`;
   if(!dodged&&run.fire&&fireLv>=3){run.enemy.burn+=Math.max(1,Math.ceil(run.fire*.5));if(fireLv>=5&&run.battleRound===3){const burst=Math.max(2,run.fire*2);run.enemy.currentHp-=burst;combatFx.enemy=`-${total+burst}`;combatFx.proc=[combatFx.proc,'🔥 烈焰爆發'].filter(Boolean).join(' · ');}}
   if(!dodged&&run.poison){const poisonMult=poisonLv>=5?2:poisonLv>=3?1.4:1;const add=Math.max(1,Math.round(run.poison*poisonMult));run.enemy.poison+=add;combatFx.proc=[combatFx.proc,`☠️ +${add}`].filter(Boolean).join(' · ')}
   if(!dodged&&run.lifesteal&&total>0){const heal=Math.max(1,Math.round(total*run.lifesteal));run.hp=Math.min(run.maxHp,run.hp+heal);combatFx.hero=`+${heal}`}
   anim='enemy-hit';render();setTimeout(()=>afterHeroAttack(profile),PAUSE.impact);},PAUSE.attack);}

function afterHeroAttack(profile){let dot=0;const tags=[];if(run.enemy.burn>0){dot+=run.enemy.burn;tags.push(`🔥 ${run.enemy.burn}`)}if(run.enemy.poison>0){dot+=run.enemy.poison;tags.push(`☠️ ${run.enemy.poison}`)}if(dot>0){run.enemy.currentHp-=dot;combatFx.enemy=`-${dot}`;combatFx.proc=tags.join(' · ')}
 if(run.enemy.currentHp<=0){anim='enemy-down';render();setTimeout(()=>finishBattleSet(true),PAUSE.defeat);return}
 clearFx();if(profile.label)combatFx.proc=profile.label;anim='enemy-turn';render();setTimeout(()=>{const enemyAtk=Math.round(run.enemy.atk*profile.enemyAtkMult),preShield=Math.max(1,enemyAtk-run.def),incoming=Math.max(1,preShield-run.shield),blocked=Math.max(0,preShield-incoming);run.hp=Math.max(0,run.hp-incoming);combatFx.hero=`-${incoming}`;
   if(profile.enemyHeal>0&&incoming>0){const heal=Math.min(profile.enemyHeal,run.enemy.hp-run.enemy.currentHp);if(heal>0){run.enemy.currentHp+=heal;combatFx.enemy=`+${heal}`;combatFx.proc=[combatFx.proc,'🩸 吸血'].filter(Boolean).join(' · ');}}
   const counterLv=skillLevel('counter'),shieldLv=skillLevel('shield');let counter=0,thorns=0,heal=0;
   if(run.counter>0){counter=counterLv>=5?Math.max(1,Math.round(run.atk*run.counter)):Math.max(1,Math.round(run.atk*run.counter)-run.enemy.def);run.enemy.currentHp-=counter;combatFx.enemy=`-${counter}`;combatFx.proc=[combatFx.proc,counterLv>=5?'↩️ 穿甲反擊':'↩️ 反擊'].filter(Boolean).join(' · ');if(counterLv>=3){heal=Math.max(1,Math.round(counter*.25));run.hp=Math.min(run.maxHp,run.hp+heal);combatFx.hero=`-${incoming} / +${heal}`;combatFx.proc+=' · 🩸 回復';}}
   if(shieldLv>=3&&blocked>0){thorns=Math.max(1,Math.round(blocked*(shieldLv>=5?.6:.3)));run.enemy.currentHp-=thorns;combatFx.enemy=`-${counter+thorns}`;combatFx.proc=[combatFx.proc,shieldLv>=5?'◇ 鋼鐵荊棘':'◇ 荊棘護體'].filter(Boolean).join(' · ');}
   anim='hero-hit';render();setTimeout(()=>{if(run.hp<=0){finish(false);return}if(run.enemy.currentHp<=0){anim='enemy-down';render();setTimeout(()=>finishBattleSet(true),PAUSE.defeat);return}if(run.battleRound>=3){finishBattleSet(false);return}run.battleRound++;anim='';clearFx();render();setTimeout(()=>playBattleRound(),PAUSE.between);},PAUSE.counter);},PAUSE.counter);}
...
function strongestBuild(){const owned=Object.entries(run.skills).sort((a,b)=>b[1]-a[1]);if(!owned.length)return '尚未形成流派';const [id,l]=owned[0],u=UPGRADES.find(x=>x.id===id),e=evolutionState(id,l).active;return `${u?.icon||'✦'} ${u?.name||id} Lv.${fmtLevel(l)}${e?`・${e.name}`:''}`;}
