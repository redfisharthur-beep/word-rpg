...
const PAUSE={review:1150,focus:500,attack:520,impact:680,counter:650,between:650,defeat:1100};
let meta=loadMeta(),screen='home',run=null,upgradeChoices=[],wheelReward=null,locked=false,anim='',answerReview=null,pendingCorrect=true;
...
function renderPlay(){const e=run.enemy,r=role(),enemyPct=clamp(e.currentHp/e.hp*100,0,100),hpPct=clamp(run.hp/run.maxHp*100,0,100);return `<main class="game-shell ${locked?'locked':''}"><header class="game-head"><button data-action="quit">×</button><div><small>STAGE ${stage().id} · CYCLE</small><b>${run.cycle}</b></div><div class="streak-chip">🔥 ${run.streak}</div></header><section class="arena ${anim}"><div class="hero-side">${visual(r.art,r.icon,'hero-art')}<div class="hero-name">${r.name}</div><div class="hpbar"><i style="width:${hpPct}%"></i></div><small>${run.hp} / ${run.maxHp}</small><div class="stats-mini"><span>⚔️ ${run.atk}</span><span>🛡️ ${run.def}</span><span>💥 ${Math.round(run.crit*100)}%</span></div></div><div class="vs-line"><span>${run.battleRound?`ROUND ${run.battleRound}/3`:'READY'}</span><b>VS</b><span>BATTLE</span></div><div class="enemy-side"><div class="enemy-label">${e.boss?'BOSS':'ENEMY'} · ${e.name}</div>${visual(e.art,e.icon,'enemy-art')}<div class="hpbar enemy"><i style="width:${enemyPct}%"></i></div><small>${Math.max(0,e.currentHp)} / ${e.hp}</small><div class="stats-mini enemy-stats"><span>⚔️ ${e.atk}</span><span>🛡️ ${e.def}</span></div>${e.poison?`<span class="effect-tag">☠️ ${e.poison}</span>`:''}</div></section><div class="skill-strip">${renderOwnedSkills()}</div>${renderQuiz()}</main>`;}
...
function renderOwnedSkills(){const owned=Object.entries(run.skills);if(!owned.length)return '<span class="empty-skill">尚未取得能力</span>';return owned.slice(-7).map(([id,l])=>{const u=UPGRADES.find(x=>x.id===id);return `<span>${u?.icon||'✦'} ${u?.name||id} <b>Lv.${fmtLevel(l)}</b></span>`}).join('');}
...
function summarizeBuild(){const owned=Object.entries(run.skills);if(!owned.length)return '尚未成形';return owned.map(([id,l])=>{const u=UPGRADES.find(x=>x.id===id);return `${u?.icon||'✦'} ${u?.name||id} Lv.${fmtLevel(l)}`}).join('　');}
...
function startBattleSet(){screen='play';locked=true;answerReview=null;run.battleRound=1;anim='';render();setTimeout(()=>playBattleRound(),PAUSE.focus);}
function playBattleRound(){if(screen!=='play'||run.battleRound<1)return;if(run.regen)run.hp=Math.min(run.maxHp,run.hp+run.regen);let raw=Math.round(run.atk*(1+run.rage*Math.max(0,run.streak-1)));if(Math.random()<run.crit)raw*=2;let damage=Math.max(1,raw-run.enemy.def)+run.fire;let combo=0;if(run.combo>0&&Math.random()<Math.min(.78,run.combo))combo=Math.max(1,Math.round(run.atk*.6)-run.enemy.def);anim='hero-attack';render();setTimeout(()=>{run.enemy.currentHp-=damage+combo;if(run.poison)run.enemy.poison+=run.poison;if(run.lifesteal){const heal=Math.max(1,Math.round((damage+combo)*run.lifesteal));run.hp=Math.min(run.maxHp,run.hp+heal)}anim='enemy-hit';render();setTimeout(()=>afterHeroAttack(),PAUSE.impact);},PAUSE.attack);}

function afterHeroAttack(){if(run.enemy.poison>0)run.enemy.currentHp-=run.enemy.poison;if(run.enemy.currentHp<=0){anim='enemy-down';render();setTimeout(()=>finishBattleSet(true),PAUSE.defeat);return}anim='enemy-turn';render();setTimeout(()=>{const incoming=Math.max(1,run.enemy.atk-run.def-run.shield);run.hp=Math.max(0,run.hp-incoming);if(run.counter>0){const counter=Math.max(1,Math.round(run.atk*run.counter)-run.enemy.def);run.enemy.currentHp-=counter}anim='hero-hit';render();setTimeout(()=>{if(run.hp<=0){finish(false);return}if(run.enemy.currentHp<=0){anim='enemy-down';render();setTimeout(()=>finishBattleSet(true),PAUSE.defeat);return}if(run.battleRound>=3){finishBattleSet(false);return}run.battleRound++;anim='';render();setTimeout(()=>playBattleRound(),PAUSE.between);},PAUSE.counter);},PAUSE.counter);}

function finishBattleSet(enemyDead){anim='';if(enemyDead){run.battleRound=0;screen='clear';locked=false;render();return}run.battleRound=0;run.cycle++;run.question=makeQuestion();locked=false;answerReview=null;screen='play';render();}

function renderStageClear(){const s=stage();return `<main class="stage-screen"><section class="stage-card clear"><small>STAGE ${s.id} CLEAR</small><div class="clear-icon">✓</div><h1>擊破 ${s.enemy}</h1><p>${s.boss?'你完成了這次冒險。':`${s.reward}，所有能力等級保留到下一關。`}</p><div class="build-line">${summarizeBuild()}</div>${s.boss?`<button class="fight-btn" data-action="finish">查看結果 <span>→</span></button>`:`<button class="fight-btn" data-action="next-stage">前往下一關 <span>→</span></button>`}</section></main>`;}

function nextStage(){const heal=Math.ceil(run.maxHp*[.12,.14,.16,.0][run.stageIndex]);run.hp=Math.min(run.maxHp,run.hp+heal);run.stageIndex++;run.cycle=1;run.battleRound=0;run.enemy=freshEnemy(stage());run.question=makeQuestion();answerReview=null;screen='stage';render();}
function finish(won){run.won=won;if(won)meta.wins+=1;meta.bestStreak=Math.max(meta.bestStreak,run.maxStreak);saveMeta();screen='end';locked=false;run.battleRound=0;render();}
