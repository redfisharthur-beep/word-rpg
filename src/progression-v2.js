...
function chineseNumber(n){const d=['零','一','二','三','四','五','六','七','八','九'];if(n<10)return d[n];if(n<20)return '十'+(n===10?'':d[n%10]);if(n<100)return d[Math.floor(n/10)]+'十'+(n%10?d[n%10]:'');return String(n)}
export function seasonOrdinal(id=currentSeasonId()){const [y,m]=String(id).split('-').map(Number),n=Math.max(1,(y-2026)*12+(m-9)+1);return `第${chineseNumber(n)}季`}
export function seasonState(){const current=currentSeasonId(),raw=load(SEASON_KEY,{season:current,points:0,wins:0,losses:0,draws:0});if(raw.season!==current){const reset={season:current,points:0,wins:0,losses:0,draws:0};save(SEASON_KEY,reset);return reset}return {season:raw.season,points:Math.max(0,Math.round(raw.points||0)),wins:Math.max(0,Math.round(raw.wins||0)),losses:Math.max(0,Math.round(raw.losses||0)),draws:Math.max(0,Math.round(raw.draws||0))}}
