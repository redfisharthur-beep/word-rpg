...
export function useUltimate(battle){
  ...
  if(role==='warrior'){
    const damage=66+(hasTalent(next,'warrior-rage')?15:0),guard=8;dealDamage(next,damage,canFinish);next.player.guard=Math.min(40,next.player.guard+guard);next.enemy.break=Math.min(next.enemy.breakMax,next.enemy.break+2);if(next.enemy.break>=next.enemy.breakMax){next.enemy.broken=true;result.broke=true;}result.amount=damage;result.enemyDamage=damage;result.ultimateText=`裂地斬・${damage} 傷害 + Break 2 + 護盾 ${guard}`;
  }else if(role==='mage'){
    const damage=42,guard=18+(hasTalent(next,'mage-starshield')?12:0);dealDamage(next,damage,canFinish);next.player.guard=Math.min(40,next.player.guard+guard);result.amount=damage;result.enemyDamage=damage;result.ultimateText=`星界爆發・傷害 + 護盾 ${guard}`;
  }else{
    const damage=60+(hasTalent(next,'archer-volley')?18:0);dealDamage(next,damage,canFinish);next.enemy.dodge=false;next.player.combo+=3;result.amount=damage;result.enemyDamage=damage;result.ultimateText=`疾風連射・${damage} 傷害 + Combo 3`;
  }
...
export function resolveAnswer(battle,answer){
  ...
  if(correct){
    ...
    if(next.player.roleId==='mage'&&q.type==='spelling'){mult=1.35;next.player.energy=Math.min(100,next.player.energy+8+(hasTalent(next,'mage-spellflow')?10:0));result.roleText=hasTalent(next,'mage-spellflow')?'法師・咒文循環':'法師・拼字共鳴';}
    if(next.player.roleId==='archer'){
      const quickLimit=hasTalent(next,'archer-swift')?9:7;if(elapsed<=quickLimit){next.player.combo+=1+(hasTalent(next,'archer-momentum')?1:0);next.player.energy=Math.min(100,next.player.energy+5);next.player.guard=Math.min(40,next.player.guard+3);mult*=1.18;result.roleText=hasTalent(next,'archer-momentum')?'弓手・乘風連擊 + 護盾 3':'弓手・迅捷連擊 + 護盾 3';}
