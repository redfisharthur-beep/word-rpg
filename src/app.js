...
const ROLES={
  warrior:{id:'warrior',name:'戰士',icon:'⚔️',art:ASSETS.role.warrior,desc:'破陣時獲得護盾',passive:'break-guard'},
  mage:{id:'mage',name:'法師',icon:'🪄',art:ASSETS.role.mage,desc:'每回合第一次答對返還 1 能量',passive:'refund'},
  archer:{id:'archer',name:'弓手',icon:'🏹',art:ASSETS.role.archer,desc:'每回合第一次答對額外造成 5 傷害',passive:'quick-shot'},
};

const CARD_POOL={
  strike:{id:'strike',name:'斬擊',icon:'⚔️',cost:1,type:'attack',text:'造成 8 傷害',boost:'答對：14 傷害',base:{damage:8},power:{damage:14}},
  guard:{id:'guard',name:'守護',icon:'🛡️',cost:1,type:'guard',text:'獲得 7 護盾',boost:'答對：12 護盾',base:{block:7},power:{block:12}},
  break:{id:'break',name:'破陣',icon:'💥',cost:1,type:'break',text:'Break +2',boost:'答對：Break +4',base:{break:2},power:{break:4}},
  heavy:{id:'heavy',name:'重擊',icon:'🔨',cost:2,type:'attack',text:'造成 16 傷害',boost:'答對：25 傷害',base:{damage:16},power:{damage:25}},
  focus:{id:'focus',name:'蓄勢',icon:'✦',cost:1,type:'skill',text:'下次攻擊 +7',boost:'答對：+12',base:{focus:7},power:{focus:12}},
  riposte:{id:'riposte',name:'反擊',icon:'↩️',cost:1,type:'guard',text:'護盾 5；敵人攻擊後反傷 5',boost:'答對：護盾 9、反傷 9',base:{block:5,riposte:5},power:{block:9,riposte:9}},
  flurry:{id:'flurry',name:'連射',icon:'➶',cost:1,type:'attack',text:'造成 5×2 傷害',boost:'答對：7×2',base:{damage:10},power:{damage:14}},
  mend:{id:'mend',name:'回息',icon:'🌿',cost:1,type:'skill',text:'回復 4 HP',boost:'答對：回復 8 HP',base:{heal:4},power:{heal:8}},
};
...
const ENEMIES={
  ...
  shadow:{id:'shadow',name:'影語王',art:ASSETS.enemy.shadowKing,icon:'👁️',hp:112,breakMax:8,intents:[{kind:'attack',value:13,label:'影襲 13'},{kind:'drain',value:9,label:'吞噬・傷害 9 / 回復 9'},{kind:'curse',value:0,label:'暗語・下回合少 1 能量'},{kind:'attack',value:23,label:'暮色爆發 23'}]},
};
...
function buildRoute(){return [
  [{type:'battle',enemy:'moss',icon:'⚔️',title:'苔蘚伏擊',sub:'普通戰鬥'},{type:'battle',enemy:'rabbit',icon:'⚔️',title:'霧角追獵',sub:'較快的敵人'}],
  shuffle([{type:'event',icon:'?',title:'迷霧事件',sub:'未知選擇'},{type:'camp',icon:'🔥',title:'營火',sub:'回復或強化'}]),
  shuffle([{type:'battle',enemy:'bubble',icon:'⚔️',title:'靜水異動',sub:'會回復的敵人'},{type:'battle',enemy:'rabbit',icon:'⚔️',title:'霧林追擊',sub:'殘影閃避'}]),
  shuffle([{type:'treasure',icon:'🎁',title:'遺物寶箱',sub:'取得永久強化'},{type:'event',icon:'?',title:'古樹低語',sub:'風險與報酬'}]),
  [{type:'elite',enemy:'beetle',icon:'☠️',title:'木甲守門者',sub:'菁英戰'},{type:'camp',icon:'🔥',title:'最後營火',sub:'為 Boss 準備'}],
  [{type:'boss',enemy:'shadow',icon:'👁️',title:'影語王',sub:'最終戰'}]
];}
function startRun(){run={role:meta.role,hp:68,maxHp:68,gold:0,score:0,step:0,route:buildRoute(),deck:freshDeck(meta.role),relics:[],battle:null,lastReward:null};screen='map';render();}
...
function renderHome(){const r=ROLES[meta.role];return `<section class="home-v2"><div class="brand-v2">WORD//RIFT <span>PROTOTYPE</span></div><div class="home-hero">${visual(r.art,r.icon,'hero-art-v2')}<div><small>選擇勇者</small><h1>${r.name}</h1><p>${r.desc}</p></div></div><div class="role-switch">${Object.values(ROLES).map(x=>`<button class="role-chip ${x.id===meta.role?'active':''}" data-role="${x.id}">${x.icon} ${x.name}</button>`).join('')}</div><button class="start-run" data-action="start-run">開始一次冒險 <b>→</b></button><div class="home-stats"><span>最佳分數 <b>${meta.best||0}</b></span><span>通關 <b>${meta.wins||0}</b></span><span>嘗試 <b>${meta.runs||0}</b></span></div><p class="prototype-note">一局約 3～5 分鐘。英文不是每一步都考；帶有 <b>WORD BOOST</b> 的卡牌才會觸發單字強化。</p></section>`;}
...
function renderMap(){const options=run.route[run.step]||[];return `${renderHeader()}<section class="map-v2"><div class="map-title"><small>CHOOSE YOUR PATH</small><h2>${run.step===5?'最後一步':'下一步走哪裡？'}</h2><p>路線不同，風險與 Build 也不同。</p></div><div class="route-progress">${[0,1,2,3,4,5].map(i=>`<i class="${i<run.step?'done':i===run.step?'now':''}"></i>`).join('')}</div><div class="node-options">${options.map((n,i)=>`<button class="route-node ${n.type}" data-node="${i}"><span>${n.icon}</span><div><b>${n.title}</b><small>${n.sub}</small></div><em>→</em></button>`).join('')}</div><div class="build-strip"><span>牌組 ${run.deck.length}</span>${run.relics.length?run.relics.map(x=>`<b>${x.icon} ${x.name}</b>`).join(''):'<b>尚無遺物</b>'}</div></section>`;}
...
function startBattle(enemyId,kind='battle'){const e=structuredClone(ENEMIES[enemyId]);if(kind==='elite')e.hp+=10;if(kind==='boss')e.hp+=8;run.battle={kind,enemy:e,enemyHp:e.hp,enemyBlock:0,enemyBreak:0,enemyDodge:false,intentIndex:0,playerBlock:0,energy:3,maxEnergy:3,hand:[],drawPile:shuffle([...run.deck]),discard:[],focus:0,riposte:0,turn:1,roleProc:false,energyPenalty:0,log:'先看敵人意圖，再決定出牌順序。'};drawHand();screen='battle';render();}
function drawHand(){const b=run.battle;b.hand=[];while(b.hand.length<4){if(!b.drawPile.length){b.drawPile=shuffle(b.discard);b.discard=[];}if(!b.drawPile.length)break;b.hand.push({uid:crypto.randomUUID?.()||`${Date.now()}-${Math.random()}`,id:b.drawPile.pop(),rune:false});}const ids=shuffle(b.hand.map((_,i)=>i)).slice(0,Math.min(2,b.hand.length));ids.forEach(i=>b.hand[i].rune=true);}
...
function answerQuiz(answer){if(!pendingQuestion||!pendingCard)return;const correct=answer===pendingQuestion.answer;const uid=pendingCard;pendingCard=null;pendingQuestion=null;playCard(uid,correct);}
...
function playCard(uid,powered){const b=run.battle,item=b.hand.find(x=>x.uid===uid);if(!item)return;const card=CARD_POOL[item.id];if(b.energy<card.cost)return;b.energy-=card.cost;const effect=powered?card.power:card.base;let notes=[];
  if(effect.damage){let dmg=effect.damage+b.focus;b.focus=0;if(run.role==='archer'&&powered&&!b.roleProc){dmg+=5;b.roleProc=true;notes.push('弓手追擊 +5');}if(b.enemyDodge){dmg=Math.ceil(dmg/2);b.enemyDodge=false;notes.push('殘影減傷');}const absorbed=Math.min(b.enemyBlock,dmg);b.enemyBlock-=absorbed;dmg-=absorbed;b.enemyHp-=dmg;notes.push(`傷害 ${dmg}`);}
  ...
  b.log=`${powered?'✓ 強化':'普通'}・${card.name}｜${notes.join('・')}`;b.discard.push(item.id);b.hand=b.hand.filter(x=>x.uid!==uid);
  if(b.enemyHp<=0){winBattle();return;}render();}

function endTurn(){const b=run.battle;if(!b)return;enemyAct();if(run.hp<=0){endRun(false);return;}b.turn+=1;b.energy=Math.max(1,b.maxEnergy-b.energyPenalty);b.energyPenalty=0;b.playerBlock=0;b.roleProc=false;drawHand();render();}
function enemyAct(){const b=run.battle,intent=currentIntent();if(b.enemy.stunned){b.enemy.stunned=false;b.log='BREAK！敵人本回合無法行動';b.intentIndex++;return;}if(intent.kind==='attack'||intent.kind==='drain'){let dmg=intent.value,blocked=Math.min(b.playerBlock,dmg);dmg-=blocked;b.playerBlock-=blocked;run.hp=Math.max(0,run.hp-dmg);if(b.riposte&&intent.value>0){b.enemyHp-=b.riposte;b.log=`敵人造成 ${dmg} 傷害・反擊 ${b.riposte}`;}else b.log=`敵人造成 ${dmg} 傷害`;if(intent.kind==='drain'){b.enemyHp=Math.min(b.enemy.hp,b.enemyHp+intent.value);b.log+=`・敵人回復 ${intent.value}`;}}
  ...
  b.riposte=0;b.intentIndex++;if(b.enemyHp<=0)winBattle();}

function winBattle(){const b=run.battle;run.score+=b.kind==='boss'?500:b.kind==='elite'?220:100;run.gold+=b.kind==='boss'?0:b.kind==='elite'?35:20;if(b.kind==='boss'){endRun(true);return;}run.lastReward=shuffle(Object.keys(CARD_POOL).filter(id=>!['mend'].includes(id))).slice(0,3);screen='reward';render();}
function renderReward(){return `${renderHeader()}<section class="reward-v2"><small>BATTLE WON</small><h2>選一張牌加入這次牌組</h2><p>這會改變後面幾場戰鬥的打法。</p><div class="reward-cards">${run.lastReward.map(id=>{const c=CARD_POOL[id];return `<button data-reward="${id}" class="reward-card"><span>${c.icon}</span><b>${c.name}</b><p>${c.text}</p><small>${c.boost}</small></button>`;}).join('')}</div><button class="skip-reward" data-action="skip-reward">不要新牌・牌組保持精簡</button></section>`;}
...
function renderEvent(){const event=run.step<3?{title:'迷霧中的商人',text:'他願意用你的生命換一件強力遺物。'}:{title:'古樹的低語',text:'樹洞裡有一股力量，但你感覺它並不安全。'};return `${renderHeader()}<section class="event-v2"><div class="event-icon">?</div><small>RANDOM EVENT</small><h2>${event.title}</h2><p>${event.text}</p><div class="event-choices"><button data-event="risk"><b>冒險</b><span>-10 HP，獲得「鋒利符文」：所有攻擊 +2</span></button><button data-event="safe"><b>保守</b><span>獲得 18 金幣，安全離開</span></button></div></section>`;}
function resolveEvent(type){if(type==='risk'){run.hp=Math.max(1,run.hp-10);if(!run.relics.some(x=>x.id==='sharp'))run.relics.push({id:'sharp',icon:'🗡️',name:'鋒利符文'});patchAttackRelic();}else run.gold+=18;advance();}
function patchAttackRelic(){if(run.relics.some(x=>x.id==='sharp')){for(const id of ['strike','heavy','flurry']){const c=CARD_POOL[id];if(!c._sharp){c.base.damage=(c.base.damage||0)+2;c.power.damage=(c.power.damage||0)+2;c._sharp=true;}}}advance();}

function renderCamp(){return `${renderHeader()}<section class="event-v2 camp"><div class="event-icon">🔥</div><small>CAMP</small><h2>營火</h2><p>只能選一個。你要保命，還是讓牌組更兇？</p><div class="event-choices"><button data-camp="heal"><b>休息</b><span>回復 22 HP</span></button><button data-camp="upgrade"><b>磨刃</b><span>隨機一張斬擊變成重擊</span></button></div></section>`;}
function resolveCamp(type){if(type==='heal')run.hp=Math.min(run.maxHp,run.hp+22);else{const idx=run.deck.indexOf('strike');if(idx>=0)run.deck[idx]='heavy';}advance();}

function renderTreasure(){const choices=[{id:'heart',icon:'♥',name:'血石',text:'最大 HP +12，並回復 12'},{id:'battery',icon:'✦',name:'裂隙電池',text:'每場第一回合 +1 能量'}];return `${renderHeader()}<section class="event-v2 treasure"><div class="event-icon">🎁</div><small>TREASURE</small><h2>只拿一件</h2><div class="event-choices">${choices.map(x=>`<button data-treasure="${x.id}"><b>${x.icon} ${x.name}</b><span>${x.text}</span></button>`).join('')}</div></section>`;}
