...
assert.doesNotMatch(app.innerHTML,/class="pet-picker-art"/,'pet choices are text only');
...
assert.equal((app.innerHTML.match(/data-pet-skill="fox\|fox-/g)||[]).length,21,'selected fox must show base and fork skills');
...
assert.equal((app.innerHTML.match(/data-pet-skill="owl\|owl-/g)||[]).length,21);
...
assert.equal((app.innerHTML.match(/data-pet-skill="dragon\|dragon-/g)||[]).length,21);
...
assert.doesNotMatch(app.innerHTML,/成就殿堂|英文學習與冒險成就/,'removed achievement headings stay hidden');
